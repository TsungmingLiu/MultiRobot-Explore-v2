...
SensorDataMapHandler::SensorDataMapHandler(ArClientBase *client,
  ...
  : SensorDataHandler(client, "getSensorDataMap", hostInfo), 
    ...
    myMapColor(hostInfo.laserColor),
...
void SensorDataMapHandler::handle(ArNetPacket *packet)
{
  ...
  //forwardMap(packet, robotID);
}

/*
void SensorDataMapHandler::forwardMap(ArNetPacket *packet, int robotID){
  myMap->cacheMap(packet, robotID);
  for(int i=0;i<map->getClientSize();i++){
    if(myMap->isForwardSuccess(robotID, i)){
      myMap->sendPacket(packet, robotID, i);
    }
  }
}*/

void SensorDataMapHandler::assignMapHandler(MoveMapHandler *mapHandler){
  myMap = mapHandler;
}
    ...
    RobotMap * map)
{
  ...
  for (unsigned int i = 0; i < clients.size(); i++) {
    if (hostsInfo[i].sensor == "laser") {
      ...
      sensorDataHandler = new SensorDataMapHandler(clients[i], hostsInfo[i]);
    }
    //sensorDataHandler->hookupRobotMap(map); // no need to hook up map anymore
    sensorDataHandlers.push_back(sensorDataHandler);
  }
}
void mapHandlersHandshake(
    std::vector<SensorDataMapHandler *> &sensorDataMapHandler,
    MoveMapHandler * mapHandler
    )
{
  for(unsigned int i = 0; i < sensorDataMapHandler.size();i++){
    sensorDataMapHandler[i]->assignMapHandler(mapHandler);
  }
}
