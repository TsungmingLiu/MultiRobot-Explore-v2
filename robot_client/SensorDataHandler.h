...
public:
  ...
  virtual void request();
  virtual void handle(ArNetPacket *packet);
  void updateRobotMap(ArNetPacket *packet);
  void assignMapHandler(MoveMapHandler *mapHandler);

private:
  void updateRobotLocation(ArNetPacket *packet, long timeStamp);
  void updateMapReadings(ArNetPacket *packet, long timeStamp);
  //void updateLaserReadings(ArNetPacket *packet, long timeStamp);
  void filterRobotLocation(MyPoint &measured);

  ArFunctor1C<SensorDataMapHandler, ArNetPacket *> myHandleFtr;
  const int myMapColor;
  const double myCosTheta;
  const double mySinTheta;
  //void forwardMap(int id, ArNetPacket *data);  // forward packet received to all clients
  MoveMapHandler *myMap;
  //MyCloud::Ptr myRobotCloudFiltered;
  //cv::KalmanFilter *myKalmanFilter;
};
